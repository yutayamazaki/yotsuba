...
mod tests {
    ...
    fn normalize_works() {
        ...
        assert_eq!(normalize(full_large_alphabets), "ABCDEFGHIJKLMNOPQRSTUVWXYZ");
        println!("{} -> {}", full_large_alphabets, normalize(full_large_alphabets));
        ...
        assert_eq!(normalize(problem), "南アルプスの天然水Sparking Lemonレモン一絞り");
        println!("{} -> {}", problem, normalize(problem));
    }
}
